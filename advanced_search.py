...
def perform_cuisine_filtered_search(collection):
    ...
    display_search_results(
        results, f"Cuisine-Filtered Results ({cuisine_filter})")
...
def perform_calorie_filtered_search(collection):
    ...
    display_search_results(
        results, f"Calorie-Filtered Results ({calorie_text})")
...
def perform_combined_filtered_search(collection):
    ...
    max_calories = int(
        max_calories_input) if max_calories_input.isdigit() else None
    ...
    filter_text = ", ".join(
        filter_description) if filter_description else "no filters"
    ...
    display_search_results(
        results, f"Combined Filtered Results ({filter_text})")
...
def display_search_results(results, title, show_details=True):
    ...
    for i, result in enumerate(results, 1):
        ...
        if show_details:
            ...
            print(
                f"   {i}. {result['food_name']} ({score_percentage:.1f}% match)")
